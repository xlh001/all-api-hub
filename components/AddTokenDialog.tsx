...
import { useTokenForm, initialFormData } from '../hooks/useTokenForm'
...
export default function AddTokenDialog(props: AddTokenDialogProps) {
  ...
  const { formData, setFormData, errors, validateForm, isEditMode, resetForm } = useTokenForm(props)
  
  const currentAccount = availableAccounts.find(acc => acc.id === formData.accountId)

  const { isLoading, availableModels, groups, resetData } = useTokenData(isOpen, currentAccount, setFormData)
