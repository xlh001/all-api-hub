...
This includes bulk channel migration, automatic backend-to-AxonHub channel conversion, and New API-only controls such as groups, priority, model sync, or model redirect unless a later requirement explicitly adds AxonHub support for those actions.

#### Scenario: AxonHub is not offered as a channel migration target
- **WHEN** the user opens an existing managed-site channel migration workflow
- **THEN** the extension MUST NOT offer AxonHub as a migration target in this change
- **AND** the extension MUST NOT automatically convert channels from other managed-site backends into AxonHub channels
