...
## Purpose
Provide a unified management experience for self-hosted **Done Hub** instances inside All API Hub, comparable to the existing managed-site support for New API (channel management, model sync, and model redirect).
...
When model redirect is enabled and `managedSiteType = done-hub`, the extension MUST be able to generate and apply a `model_mapping` update for eligible channels.

#### Scenario: Applying model redirect writes model_mapping for Done Hub channels
- **WHEN** the user runs model redirect with `managedSiteType = done-hub`
- **THEN** the extension MUST compute a standard-model-to-actual-model mapping per channel
