...
export default function ModelList({
  ...
  const scrollToSelectedTab = (selectedIndex: number) => {
    ...
    if (selectedIndex >= 0 && selectedIndex < tabs.length) {
      ...
      const idealScrollLeft =
        tabLeft - containerWidth / 2 + selectedTabRect.width / 2
  ...
  const availableGroups = useMemo(() => {
    ...
    const groups = Object.keys(pricingData.group_ratio).filter(
      (key) => key !== ""
    )
            ...
            selectedIndex={(() => {
              const index =
                selectedProvider === "all"
                  ...
                  : Math.max(
                      0,
                      providers.indexOf(selectedProvider as ProviderType) + 1
                    )
              console.log(
                "当前 selectedProvider:",
                selectedProvider,
                  ...
                  {filteredModels.length === 0 ? (
                    ...
                    filteredModels.map((item, index) => (
                      <ModelItem
                        ...
                        exchangeRate={
                          currentAccount?.balance?.USD > 0
                            ? currentAccount.balance.CNY /
                              currentAccount.balance.USD
                            : 7
                        }
