...
| 系统类型 | 同步逻辑 |
|----------|----------|
| **New API / DoneHub / Veloera** | 调用 `GET /api/channel/fetch_models` 获取上游，再更新渠道配置。 |
| **Octopus** | 调用 `POST /api/v1/channel/fetch-model` 获取上游模型。 |
| **AxonHub** | 通过 AxonHub 管理接口自动读取并同步渠道关联的模型列表。 |

## 前置要求

在开始同步前，你需要在 **基础设置 -> 自建站点管理** 中完成对应系统的连接配置（Base URL、管理员令牌 `Admin Token` 或账号密码）。
...
| **每分钟请求数（RPM）** | 20 | 每分钟允许的最大请求数，用于保护上游站点。 |
...
| 同步导致部分模型丢失 | 默认情况下，同步会**覆盖**现有的模型列表。如果你有自定义模型（非上游返回），请在同步后手动补录或使用白名单策略（测试版）。 |
