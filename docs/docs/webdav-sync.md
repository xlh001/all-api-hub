...
请先暂停自动同步，再把共享同一 WebDAV 文件的设备全部升级到支持 V7 账号的版本，最后重新启用同步。不支持 V6 与 V7 客户端同时写入同一个备份文件；并发写入时，whole-account LWW 可能让旧客户端用 V6 账号覆盖 V7 签到配置。若必须回退旧版，请先停用其他设备的同步，并保留独立备份文件。
