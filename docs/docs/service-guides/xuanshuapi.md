...
# 玄枢API 用户如何用 All API Hub 统一管理 AI API 资产

> 在 All API Hub 中管理玄枢API 账号：统一看余额、对比价格、管理密钥，并快速配置到常用 AI 工具
...
**All API Hub**（[GitHub 开源](https://github.com/qixing-jk/all-api-hub)）是一款面向 AI API 用户的开源浏览器扩展，适合用来集中管理多个账号、多个站点和多个客户端配置。对玄枢API 用户来说，它可以把玄枢API 的账号信息、API 密钥、模型价格和导出配置纳入统一工作流。

配合玄枢API 使用时，核心优势在于：

*   **多账号统一看板**：把玄枢API 与其他 AI API 账号放在同一页，集中查看余额、状态和刷新结果。
*   **跨账号价格对比**：在模型价格页查看玄枢API 模型价格，并与其他已添加账号的数据一起比较。
*   **API 密钥集中管理**：在密钥管理中查看、创建、编辑、删除和复制玄枢API 的 API Key。
*   **凭据快速复用**：把已管理的 `Base URL + API Key` 导出到常用客户端、CLI 工具或自建站点渠道。
*   **多设备更好衔接**：配合数据导入导出或 WebDAV 同步，把常用配置迁移到其他设备继续使用。

一句话：玄枢API 提供模型和接口，All API Hub 帮你把多账号管理、价格对比和下游工具配置串起来。
...
All API Hub 支持自动识别玄枢API 账号。你只需要先在浏览器里登录玄枢API，再让插件读取当前站点并保存账号。

### 为什么适合玄枢API 用户？

玄枢API 适合接入多种模型和客户端。加入 All API Hub 后，你可以把玄枢API 放进统一的 AI API 管理流程：

*   与其他账号一起查看余额、状态和模型价格。
*   在模型价格页对比不同账号下的模型成本。
*   在插件中直接管理玄枢API 的 API 密钥，包括创建、编辑、删除和复制。
*   将玄枢API 密钥继续导出到 AI 客户端，或导入到你自己的自建站点渠道中。

对于已经在多个 AI 工具中使用玄枢API 的用户，这相当于把“账号状态、模型价格、API Key、客户端配置”整理成一条更顺的使用链路。

### 3.1 自动识别并添加
1.  在浏览器中登录 [玄枢API](https://www.xuanshuapi.com/register?aff=ALL-API-HUB&promo=ALL-API-HUB)。
2.  点击浏览器右上角的 All API Hub 扩展图标。
3.  点击 **“添加账号”**，使用当前站点地址或手动填写玄枢API 地址。

    ![填写玄枢API 控制台地址并准备自动识别](../static/image/sponsor-guides/xuanshuapi/xuanshuapi-add-account-auto-detect.png)

4.  点击 **“自动识别”**。
5.  确认账号信息后，点击 **“保存账号”**。

    ![确认识别出的玄枢API 账号信息](../static/image/sponsor-guides/xuanshuapi/xuanshuapi-account-details-confirm.png)

:::: tip 提示
添加成功后，扩展会使用已导入的账号令牌读取余额、API 密钥和模型价格等信息。
::::

### 3.2 管理玄枢API 的 API 密钥
账号添加成功后，你可以进入 **“密钥管理”** 页面，集中管理玄枢API 的 API 密钥：
...
如果你的目标只是日常查看和整理玄枢API 密钥，使用 **“密钥管理”** 就足够；当你要把同一份密钥继续配置到其他客户端、CLI 工具或自建后台时，再使用导出能力。
...
### 4.1 查看余额与账号状态
在 All API Hub 的首页看板中，你可以把玄枢API 与其他 AI API 账号放在一起查看。余额、账号状态和刷新结果集中展示，适合多账号用户快速了解当前可用情况。

### 4.2 对比模型价格
进入 **“模型价格”** 页面，选择玄枢API 账号作为数据源。你可以：
...
![查看玄枢API 模型列表与价格](../static/image/sponsor-guides/xuanshuapi/xuanshuapi-model-price-list.png)

### 4.3 导出到 AI 客户端
如果你需要将玄枢API 接入其他工具，可以直接从 All API Hub 导出：
...
| --- | --- |----------------------------------|
| **核心定位** | 统一管理玄枢API 与其他 AI API 账号、余额、密钥、价格和渠道 | 发起对话、模型推理、提示词工程 |
| **主要功能** | 多账号看板、密钥管理、价格对比、凭据导出、渠道导入 | 聊天对话、文件分析、Agent 工作流 |
| **协同关系** | **整理源头配置**：让 Key、Base URL、价格和账号状态集中可用 | **使用这些配置**：拿管理好的凭据去调用模型 |

**建议用法**：用 All API Hub 管理玄枢API 账号、密钥、模型价格和导出配置；用你常用的客户端实际发起请求。一个负责管配置，一个负责用模型。
...
**Q: All API Hub 最适合哪些玄枢API 用户？**

A: 如果你有多个玄枢API 账号、同时使用其他 AI API 平台，或经常把玄枢API 配置到多个客户端和设备里，All API Hub 可以把这些账号与凭据集中起来管理。即使只从玄枢API 开始使用，也可以先用它查看余额、管理密钥和对比模型价格。

**Q: 没有自建后台，也能使用 All API Hub 吗？**

A: 可以。添加玄枢API 账号后，就可以使用余额查看、密钥管理、模型价格对比和客户端导出；自建站点管理适合已经维护 AI 分发后台的用户继续扩展使用。
...
A: 两者是配合关系。玄枢API 控制台负责账号、充值和官方服务；All API Hub 更适合把玄枢API 的账号状态、API Key、模型价格和客户端配置纳入你的日常管理流程。

---

## 结语

玄枢API 提供丰富的模型与 API 调用入口，All API Hub 则让这些账号、密钥、价格和客户端配置更容易统一管理。

安装插件并添加玄枢API 账号后，你可以先从三个最常用的动作开始：查看余额、对比模型价格、管理密钥与导出到常用客户端。后续如果你需要接入自建后台、多设备同步或批量管理，再逐步启用更完整的管理能力。

*   [玄枢API 官网](https://www.xuanshuapi.com/register?aff=ALL-API-HUB&promo=ALL-API-HUB)
