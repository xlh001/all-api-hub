...
添加 PackyCode 账号后，你可以在 All API Hub 中查看余额、管理 API 密钥、查询模型价格，并快速导出到 Cherry Studio、CC Switch、Kilo Code、Claude Code Router 或 CLIProxyAPI、New API、Sub2API 等自建网关。这样，PackyCode 可以自然接入你的多账号、多工具 API 使用流程。
...
### 4.4 导入到自建站点渠道
如果你维护着 CLIProxyAPI、New API、Sub2API 等自建网关，可以把 PackyCode 作为其中一个上游供应商。All API Hub 可以把 PackyCode 密钥作为上游渠道直接导入进去，减少手动创建渠道、填写地址和复制 Key 的步骤。
