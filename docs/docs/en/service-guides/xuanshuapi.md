# Managing XuanShu API Assets with All API Hub

> Use All API Hub with XuanShu API to check balances, compare model pricing, manage API keys, and export credentials to the AI tools you already use.
...
**All API Hub** ([open source on GitHub](https://github.com/qixing-jk/all-api-hub)) is a browser extension for AI API users who need to manage multiple accounts, sites, and client configurations. For XuanShu API users, it brings XuanShu API account status, API keys, model pricing, and export actions into one workflow.

When used with XuanShu API, it helps with:

- **Unified multi-account dashboard**: view XuanShu API together with other AI API accounts.
- **Cross-account price comparison**: compare XuanShu API model prices with other added accounts.
- **Centralized API key management**: view, create, edit, delete, and copy XuanShu API keys.
...
XuanShu API provides the model API, and All API Hub helps organize the account, key, pricing, and downstream tool configuration around it.
...
## 3. Add a XuanShu API Account

All API Hub can auto-recognize XuanShu API accounts. Log in to XuanShu API in your browser first, then let the extension read the current site and save the account.

### 3.1 Auto-Recognize and Add

1. Log in to [XuanShu API](https://www.xuanshuapi.com/register?aff=ALL-API-HUB&promo=ALL-API-HUB) in your browser.
2. Click the All API Hub extension icon in the top-right corner of the browser.
3. Click **Add Account**, then use the current site address or manually enter the XuanShu API address.

   ![Enter the XuanShu API console address before auto-recognition](../../static/image/sponsor-guides/xuanshuapi/xuanshuapi-add-account-auto-detect.png)

4. Click **Auto-Recognize**.
5. Confirm the account information, then click **Save Account**.

   ![Confirm the recognized XuanShu API account information](../../static/image/sponsor-guides/xuanshuapi/xuanshuapi-account-details-confirm.png)
...
### 3.2 Manage XuanShu API Keys

After adding the account, open **Key Management** to manage XuanShu API keys:
...
Use **Key Management** for day-to-day XuanShu API key organization. Use export actions when you want to reuse the same key in a client, CLI tool, or self-hosted backend.

---

## 4. Common XuanShu API Workflows

### 4.1 View Balance and Account Status

The All API Hub dashboard shows XuanShu API alongside your other AI API accounts. Balance, status, and refresh results are shown in one place, which is useful when you manage multiple accounts.

### 4.2 Compare Model Pricing

Open **Model Pricing** and select the XuanShu API account as the data source. You can:

- View the model list returned by XuanShu API.
- Search for a model and test whether it is available.
- Check input and output pricing for each model.
- Compare XuanShu API pricing with models from other added accounts.

![View XuanShu API model list and pricing](../../static/image/sponsor-guides/xuanshuapi/xuanshuapi-model-price-list.png)

### 4.3 Export to AI Clients

To use XuanShu API in another tool:
...
| --- | --- | --- |
| Core role | Manage XuanShu API and other AI API accounts, balances, keys, pricing, and channels | Send chats, run inference, and manage prompts or agent workflows |
...
Recommended workflow: manage XuanShu API accounts, keys, pricing, and exports in All API Hub, then use your preferred client to send requests.
...
A: It is useful if you have multiple XuanShu API accounts, also use other AI API platforms, or often configure XuanShu API in several clients and devices. You can still start with one XuanShu API account and use balance checks, key management, and pricing comparison.

**Q: Can I use All API Hub without a self-hosted backend?**

A: Yes. After adding a XuanShu API account, you can view balances, manage keys, compare prices, and export to clients. Self-hosted site management is only needed if you maintain an AI distribution backend.
...
**Q: What is the relationship between All API Hub and the XuanShu API console?**

A: They work together. The XuanShu API console remains the source for account, recharge, and official service operations. All API Hub is better suited for day-to-day account status, API key, pricing, and client configuration management.

---

## Links

- [XuanShu API](https://www.xuanshuapi.com/register?aff=ALL-API-HUB&promo=ALL-API-HUB)
