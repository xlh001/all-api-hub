...
> ローカルの JSON ファイルでアカウントと設定をバックアップするか、WebDAV または GitHub Secret Gist で端末間の同期を行えます。
...
**インポート/エクスポート → クラウド同期**で WebDAV または GitHub Secret Gist を選び、手動同期または定期同期を利用できます。同期方針はマージ、アップロードのみ、ダウンロードのみから選択できます。設定手順は [クラウド同期](./webdav-sync.md) を参照してください。
