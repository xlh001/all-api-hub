...
| 初回取得 | サイトの初回取得はベースラインの記録のみを行います。そのとき返された公告はすべて履歴として扱われ、既読で保存され、通知されません。 |
| 通知の対象期間 | 設定した期間より前に公開された公告は既読として保存され、通知されません。期間は設定で変更できます。 |
| 上流の既読同期 | 公告ページで既読にするとサイト側にも同期されます。通知送信成功時にも同期するかどうかは設定で切り替えます。 |
| ローカル保持 | 各サイトは直近の公告記録のみを保持し、古いものから順に破棄されます。 |
