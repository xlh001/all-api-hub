...
### メッシュグラデーションラボ (Mesh Gradient Lab) とは何ですか？

**メッシュグラデーションラボ** は、プラグインに組み込まれた開発者向けツールで、主に [シェアスナップショット](./share-snapshot.md) 機能で使用される動的な背景のデバッグとプレビューを目的としています。

- **機能**: シード値 (Seed)、パレットインデックス (Palette Index)、レイアウトインデックス (Layout Index) を手動で調整し、さまざまな組み合わせによる視覚効果をリアルタイムでプレビューできます。
