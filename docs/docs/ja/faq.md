...
2. **手動追加**：自動認識に失敗した場合は、[アカウント手動追加ガイド](./add-account.md#manual-addition)に従ってサイト種別を選び、対応する項目を確認してください。
   - New API と互換サイト：サイト名、ユーザー名、ユーザー ID、Access Token、チャージレート
   - Sub2API：サイト名、ユーザー ID、JWT Access Token、チャージレート。拡張機能管理セッションを有効にする場合は Refresh Token も必要です
   - OpenRouter：サイト名、Management Key、チャージレート。ユーザー ID は不要で、Cookie モードにも対応していません
...
より完全なエクスポートと統合の説明については、[クイックサイト設定のエクスポート](./quick-export.md)を参照してください。CLIProxyAPI管理インターフェースとの連携を希望する場合は、[CLIProxyAPI の管理](./cliproxyapi-integration.md)を参照してください。

### サイトチェックイン機能の使用方法を教えてください。
