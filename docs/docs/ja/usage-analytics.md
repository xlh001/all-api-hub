...
### 同期ステータス管理 (Sync Status)

「**設定 → 基本設定 → アカウント使用量 → 同期ステータス**」では、「ダウンロードタスク」を管理するように使用量の同期を管理できます。

- **ステータス概要**：どのアカウントが同期に成功し、どれが権限やネットワークの理由で失敗したかを一目で確認できます。
- **差別化された同期**：特定の期間のみの同期や、特定のアカウントに対する強制同期のトリガーをサポートしています。
- **ワンクリッククリーンアップ**：ローカルデータが多すぎる場合や異常が発生した場合は、ワンクリックで特定のアカウントのローカル使用量キャッシュをクリアできます。
- **エラー修正**：同期が失敗した場合は、ステータスアイコンをクリックして、特定の API エラーの詳細（例：`401 Unauthorized` は管理トークンが無効であることを示します）を表示できます。

## ステップ 2：利用状況分析ページに移動する
