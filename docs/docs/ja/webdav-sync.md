...
アカウントと設定は WebDAV で同期できますが、自動チェックインの**当日の実行状態、最新結果、ブラウザのアラームはデバイス間で共有されません**。複数のデバイスで毎日のスケジュールを有効にすると、重複してチェックインする可能性があります。避けるには、自動チェックインのスケジュールを 1 台のデバイスだけで有効にしてください。
