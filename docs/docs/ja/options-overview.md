...
- 概要には最初に短い紹介が表示されます。「手順を見る」から展開できます。閲覧、展開、または「設定ガイド」からの移動だけではガイドは開始されません。
- アカウント追加、認証情報の保存、ゲートウェイ設定、チャネルのインポートなどの具体的な操作を選ぶと、開始済みとして記録されます。その後、関連するアカウントや認証情報のページに次の手順が表示されます。
- ガイドはいつでも折りたたむことができ、表示状態は保存されます。開始後は「設定を続ける」から進捗を確認できます。
- チャネルの作成・インポートに成功するか、チャネル管理で既存チャネルの読み込みに成功すると完了になります。完了したガイドは概要で既定では非表示になり、「設定ガイド」から再表示できます。
- ゲートウェイ設定の説明と、ゲートウェイ未設定時のチャネルページにも「設定ガイド」があり、概要のガイド全体が見える位置に移動します。
