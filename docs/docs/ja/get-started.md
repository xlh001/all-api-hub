...
インストール方法は README と同じで、主に 2 つの方法があります：

1. GitHub Release から `all-api-hub-<version>-safari-xcode-bundle.zip` をダウンロードし、解凍後に Xcode プロジェクトを直接開いて実行します。
2. ソースコードからビルド：`pnpm install` -> `pnpm run build:safari` -> `xcrun safari-web-extension-converter .output/safari-mv2/` -> Xcode で実行します。
