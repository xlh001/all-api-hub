...
- **設定 → API 資格情報** で、`Base URL`、API キー、タグ、メモを個別に保存できます。これは、「サイトアカウントがなく、インターフェース設定のみがある」シナリオに適しています。
- 残高と今日の使用量の概要を表示し、インターフェース/CLI の互換性を検証できます。また、モデルリストから現在の資格情報を直接開くこともできます。
- 各資格情報から CherryStudio、CC Switch、Kilo Code、CLIProxyAPI、Claude Code Router、または現在のセルフホスト型サイトに直接エクスポートすることもできます。

### 5.7 ブックマーク管理

- **設定 → ブックマーク** で、コンソール、ドキュメント、チャージページ、交換ページなどのショートカットリンクを保存できます。
- 検索、タグフィルタリング、ピン留め、ドラッグアンドドロップによる並べ替えをサポートしています。ポップアップウィンドウの上部から `ブックマーク` ビューにすばやく切り替えることもできます。
- 完全なアカウントを作成するほどではないが、一般的なエントリポイントをまず一元的に整理したい場合に適しています。

### 5.8 残高履歴

- **設定 → 基本設定 → 残高履歴** で有効にすると、残高、収入、支出のスナップショットを毎日記録できます。
- **設定 → 残高履歴** ページで概要、トレンドグラフ、アカウント分布、アカウントサマリーテーブルを表示します。
- 長期的な残高変動、チャージ消費ペースの把握、または簡単な照合に適しています。

### 5.9 使用量分析

- まず **設定 → 基本設定 → アカウント使用量** で使用量履歴の同期を有効にしてから、**設定 → 使用量分析** でグラフを表示します。
- サイト、アカウント、トークン、日付ごとにフィルタリングして、リクエスト数、トークン消費量、モデル分布、コスト、遅延の傾向を表示できます。
- 「最近どこで最も使用し、最も費用がかかり、どのリンクが最も遅いか」といった質問に答えるのに適しています。

### 5.10 セルフホスト型サイトモデル同期

セルフホスト型サイトモデル同期機能の詳細については、[セルフホスト型サイトモデル同期](./managed-site-model-sync.md) を参照してください。

### 5.11 セルフホスト型サイト管理（Beta）

拡張機能内で直接チャネルを作成/編集/削除し、モデルホワイトリストと単一チャネル同期デバッグと組み合わせることで、New API バックエンドへの往復回数を大幅に削減できます。詳細な操作と注意事項については、[セルフホスト型サイト管理](./self-hosted-site-management.md) を参照してください。

<a id="cloudflare-window-downgrade"></a>
### 5.12 Cloudflare 保護と一時ウィンドウのダウングレード
...
- **キー管理** ページですべてのアカウント配下のトークンを一元的に表示し、検索やワンクリックコピーをサポートします。
- New API サイトの場合、管理画面で非表示になっているフルキーを自動的に補完する **「欠落したキーを修復」** 機能をサポートしています。
- トークンを独立した API 認証情報として素早く保存したり、CLI 互換性を一括検証したりできます。詳細は [キー管理](./key-management.md) を参照してください。

### 5.14 シェアスナップショット

- 概要パネルや個別アカウントのステータスを、プライバシー保護のために脱敏（脱感作）処理を施した精美な画像として生成できます。
- API キーなどの機密情報を自動的に隠し、動的なメッシュグラデーション背景を内蔵しているため、コミュニティへの共有に最適です。
- 詳細は [シェアスナップショット](./share-snapshot.md) を参照してください。

### 5.15 LDOH サイト検索

- [Linux.do](https://linux.do) コミュニティのデータに基づき、お使いのサイトに対応するコミュニティの議論投稿を自動的にマッチングします。
- 中継サイトの評判、評価、最新動向を素早く把握するのに役立ちます。
- 詳細は [LDOH サイト検索](./ldoh-site-lookup.md) を参照してください。

### 5.16 New API 安全検証 (2FA / OTP)

- セルフホスト型サイト管理で機密情報を表示したり、高度な操作を実行したりする際、拡張機能が OTP 検証を案内します。
- 2FA、安全検証コード、および Passkey の手動案内をサポートしています。
- 詳細は [New API 安全検証](./new-api-security-verification.md) を参照してください。

### 5.16 Web AI API スニッフィングと検証

- ウェブページ上の Base URL または API Key を選択すると、右クリックメニューからインターフェースの可用性を直接テストできます。
- ホワイトリストに登録されたサイトでのバックグラウンド自動検出とクイック検証をサポートしています。
- 詳細は [Web AI API スニッフィングと検証](./web-ai-api-check.md) を参照してください。
...
- [ブックマーク管理](./bookmark-management.md)
- [API 資格情報](./api-credential-profiles.md)
- [キー管理](./key-management.md)
- [シェアスナップショット](./share-snapshot.md)
- [残高履歴](./balance-history.md)
- [使用量分析](./usage-analytics.md)
- [LDOH サイト検索](./ldoh-site-lookup.md)
- [Web AI API スニッフィングと検証](./web-ai-api-check.md)
- [New API 安全検証 (2FA / OTP)](./new-api-security-verification.md)
