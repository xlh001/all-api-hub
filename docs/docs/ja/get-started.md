...
- 詳細については、[Cloudflare シールド回避アシスタント](./cloudflare-helper.md) を参照してください。
...
- **5.13 [New API 安全検証](./new-api-security-verification.md)**：管理バックエンドの OTP や Passkey 等の認証チャレンジに対応。
...
- **5.20 [開発者ツール](./developer-tools.md)**：視覚的なデバッグと背景のカスタマイズ（メッシュグラデーションデバッグツール）。
