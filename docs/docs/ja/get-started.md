...
### 📊 資産ダッシュボードと統計
- **5.1 資産概要とリアルタイム更新**：複数サイトの残高、使用量、健全状態を一元管理。自動同期をサポート。
- **5.2 残高履歴**：日々の残高、収入、支出を記録し、資産の変動トレンドを直感的に把握。
- **5.3 使用量統計分析**：トークン消費、モデル分布、コスト、応答遅延を多角的に分析。

### 🔑 キー管理とクイック連携
- **5.4 キー管理 (Tokens)**：トークンを一元管理。管理画面で非表示のキーをワンクリックで復元する機能を搭載。
- **5.5 独立した API 資格情報**：アカウントを作成せずに URL+Key を保存。タグ管理や一括検証が可能です。
- **5.6 外部ツールへの高速同期**：CherryStudio、CC Switch、Kilo Code 等へ設定を素早くエクスポート。
- **5.7 ウェブ API スニッフィング**：閲覧中のページから API 設定を識別・テストし、そのまま保存可能。

### ⚡ 自動化と特典獲得
- **5.8 自動チェックイン**：サインイン対応サイトを一括処理。定期実行やカスタム URL 遷移をサポート。
- **5.9 引き換えアシスタント (Redemption Assist)**：ページ上の引き換えコードを自動認識し、ワンクリックで適用。
- **5.10 ブックマーク整理**：AI 関連のコンソール、ドキュメント、チャージページを一元管理。

### 🛡️ 安定性とセキュリティ
- **5.11 Cloudflare 盾越えアシスタント**：5秒ルール等のチャレンジを自動支援。API 呼び出しの中断を防ぎます。
- **5.12 2段階認証の対応 (2FA)**：管理バックエンドの OTP や Passkey 等の認証チャレンジにスムーズに対応。
- **5.13 WebDAV 同期と暗号化**：データのクロスデバイス暗号化バックアップをサポートし、設定を保護。

### 🛠️ 管理者向け効率化ツール
- **5.14 チャネル管理**：拡張機能内から直接 New API、AxonHub 等のチャネルを追加・編集・削除。
- **5.15 モデル同期と正規化**：アップストリームからのモデル取得と、「標準モデル名」へのマッピング設定。

### 🎨 カスタマイズと高度な設定
- **5.16 表示順序の設定**：残高、健全度、サインインの必要性等に基づき、アカウントの並び順を調整。
- **5.17 シェアスナップショット**：機密情報を隠した精美なステータス画像を生成。動的背景でSNS共有に最適。
- **5.18 サイトの評判検索**：Linux.do コミュニティのデータに基づき、サイトの評判やレビューを自動表示。
- **5.19 開発者ラボ**：視覚的なデバッグと背景のカスタマイズ（Mesh Gradient Lab）。詳細は [開発者ツール](./developer-tools.md) を参照。
...
- [アカウントの管理と維持](./account-management.md)
