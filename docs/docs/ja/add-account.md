...
拡張機能のポップアップでは **「サイドパネルで続行」** をクリックすると、入力中のフォームを引き継げます。サイドパネルに対応していない場合は **「通常のページで続行」** をクリックしてください。サイドパネルを開けない場合も通常のページで続行します。続いて **「サイトのセキュリティ設定を開く」** を選び、サイトで認証してトークンをコピーし、保持されたフォームに貼り付けて保存してください。
:::
