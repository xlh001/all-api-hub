...
- 自動チェックインの**毎日のスケジュール実行**と**自動再試行**が完了すると、タスク結果通知が送信されます。
- 自動チェックインページの**「今すぐ実行」**と、アカウントごとの**「クイックチェックイン」**では、スケジュールタスク通知は送信されません。これらは手動実行です。
- 自動チェックイン設定の**「自動チェックイン完了後にデータと画面を更新」**は通知チャネルのスイッチではありません。チェックイン後に画面データを更新するかどうかだけを制御します。
