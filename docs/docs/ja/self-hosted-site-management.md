...
| ------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[CLIProxyAPI](./cliproxyapi-integration.md)**                 | プロバイダー         | 上流プロバイダー、キー、モデルマッピングを管理します。                                                                                               |
...
| **認証資格情報** | **CLIProxyAPI**: 管理キー。<br>**New API シリーズ**: 管理者トークン (Admin Token) とユーザー ID が必要です。<br>**Sub2API**: 管理者 API Key（キーの取得に追加のウェブ認証が必要な場合、拡張機能ではキーの表示・エクスポートや他のサイトへのチャネル移行はできません）。<br>**AxonHub**: 管理者メールアドレスとパスワード。<br>**Claude Code Hub**: 管理者メールアドレスとパスワード。<br>**Octopus**: ユーザー名とパスワード。 |
...
1. **チャネルを追加**をクリックするか、既存のチャネルの編集画面を開きます。
2. システムに応じて接続先、モデル、グループなどを入力し、必要に応じて詳細設定を開きます。
...
3. キー欄でキーを入力・変更します。New API、Octopus、AxonHub、CLIProxyAPI の対応チャネルでは複数キーを個別に管理でき、変更しないキーは保持されます。New API ではランダムまたはラウンドロビンも選択できます。
4. 内容を確認して保存します。既存キーの読み取りで認証を求められた場合は、案内に従って完了してください。
