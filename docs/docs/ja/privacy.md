...
本拡張機能は、**いかなる個人識別データや機密性の高いアカウントデータも収集せず、当社のサーバーに送信しません**。お客様が追加および管理するアカウントデータは、主にアカウント表示、残高更新、キー管理、モデル管理、使用量分析、自動サインイン、サイト公告、および同期などの機能のために使用されます。
...
本拡張機能をアンインストールすると、ブラウザが当社のホストするアンケートページ（`https://all-api-hub.qixing1217.top/uninstall.html`）を開き、アンインストールの理由を伺います。このページは匿名製品統計と同じ境界に従います：

*   ページのアドレスに匿名統計IDが含まれるのは、匿名製品統計が有効な場合のみです。これは、今回のアンインストールとそれ以前の機能の利用状況を関連付けるためです。統計を無効にしている場合もページは通常どおり開きますが、アドレスにこのIDは含まれず、ページを開いた時点でもイベントは送信されません。
*   アドレスには、拡張機能のバージョン、インストールからアンインストールまでの日数、およびインターフェース言語も含まれます。これは「インストール直後にアンインストールした」場合と「長期間使用した後にアンインストールした」場合を区別するためです。
*   アンインストール理由が送信されるのは、ページ上で理由を選択して送信をクリックした場合のみです。説明文は、お客様自身が入力して送信した場合にのみ送信され、自動的に収集されることはありません。
*   アンケートの回答と匿名製品統計は、同じ PostHog プロジェクトに保存されます。
*   匿名製品統計を無効にしていても、送信をクリックすればフィードバックは開発者に送信されます。ただし、いかなる匿名統計IDとも関連付けられません。
...
これらのサービスとのすべての通信は、お客様によってトリガーされるか、本拡張機能のバックグラウンドタスクがお客様の設定に従って実行され、対応する機能を実現するためにのみ使用されます。
