...
- **クイックエクスポート**：現在設定されているセルフホスト型サイトへの直接操作を優先表示します。その他の対象は、チャットクライアント、コーディングエージェント、ゲートウェイとルーティングツールに分かれ、Cherry Studio、Kelivo、CC Switch、Kilo Code / Roo Code、Cursor++、Claude Code Router に対応します。
...
**`テスト方式`** で、**`ストリーミング`**（既定）または **`非ストリーミング`** を選択します。生成系テストは選択した方式で実行され、結果と新しく保存される履歴に使用した方式が表示されます。モデル一覧の取得には影響しません。エンドポイントがリクエストを拒否した場合は、方式を切り替えて再テストできます。テスト中に方式が自動で切り替わることはありません。
...
**`テスト方式`** で、**`ストリーミング`**（既定）または **`非ストリーミング`** を選択します。各 CLI に対応する API 種別でツール呼び出しを確認し、結果にテスト方式を表示します。失敗時に方式が自動で切り替わることはありません。
...
セルフホスト型サイト以外の対象は **`エクスポート`** メニューにまとめられています。受け渡し方法はツールごとに異なり、deeplink、クリップボード、設定内容を使うものに加え、Claude Code Router はリモート管理 API を呼び出します。統一された「安全な設定ファイルのエクスポート」ではありません。
