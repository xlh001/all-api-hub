...
本扩展 **不会收集或向我们的服务器传输任何个人身份数据或敏感账户数据**。您添加和管理的账户数据主要用于本机功能，包括账号展示、余额刷新、密钥管理、模型管理、用量分析、自动签到、网站公告和同步等。
...
当你卸载本扩展时，浏览器会打开一个由我们托管的问卷页面（`https://all-api-hub.qixing1217.top/uninstall.html`），用于了解卸载原因。该页面遵守与匿名产品统计相同的边界：

* 只有在匿名产品统计开启时，页面地址才会携带匿名统计 ID，用于把这次卸载与卸载前的功能使用情况关联；关闭统计时页面照常打开，但地址中不包含该 ID，页面打开时也不会上报任何事件。
* 地址中还会携带扩展版本、从安装到卸载的天数以及界面语言，用于区分「装上后很快卸载」与「长期使用后卸载」这类差异。
* 只有你在页面上选择卸载原因并点击提交后，才会发送该原因；说明文本仅在你主动填写并提交时发送，不会自动采集。
* 问卷反馈与匿名产品统计存放在同一个 PostHog 项目中。
* 即使你关闭了匿名产品统计，主动点击提交仍会把反馈发送给开发者，只是不会与任何匿名统计 ID 关联。
...
所有与这些服务的通信均由您主动触发，或由本扩展的后台任务按您在设置中的配置触发，仅用于实现对应功能。
