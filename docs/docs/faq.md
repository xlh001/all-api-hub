...
### 什么是网格渐变实验室 (Mesh Gradient Lab)？

**网格渐变实验室** 是插件内置的一个开发者工具，主要用于调试和预览 [分享快照](./share-snapshot.md) 功能所使用的动态背景。

- **功能**：你可以手动调整种子值（Seed）、调色盘索引（Palette Index）和布局索引（Layout Index），实时预览不同组合下的视觉效果。
