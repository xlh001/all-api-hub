...
- 已在插件中配置好 **New API 集成设置**：
  - 填写 **管理员 URL**、**管理员令牌 `Admin Token`**、**用户 ID**。
...
4. 对每个渠道：
   ...
   - 对每个标准模型执行多阶段匹配：
     ...
     - **版本守卫**：只允许匹配同版本模型，不会跨版本降级/升级（例如不会将 `claude-4.5-sonnet` 映射到 `claude-3.5-sonnet`；找不到同版本候选则跳过该标准模型）。
...
- **提示 New API 配置缺失**
  - 请先在 **“基础设置 → New API 集成设置”** 中填写管理员 URL、管理员令牌 `Admin Token` 和用户 ID 并保存。
- **提示功能未启用**
  - 确认已在“模型重定向”区域打开“启用模型重定向”开关。
- **某些标准模型未生成映射**
  - 说明在对应渠道中未找到可匹配的实际模型，或所有候选都已被其它标准模型占用。
  - 也可能是该渠道没有**同版本**候选模型：插件不会跨版本降级/升级生成映射。
