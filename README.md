...
All API Hub 作为浏览器扩展，一站式管理 New API 等中转站账号与独立 API 凭证：余额/用量看板、模型价格比对、自动签到、密钥一键导出到常用应用、网页内 API 可用性测试、渠道与模型同步/重定向，并为自建 New API、DoneHub、Veloera、Octopus 与 AxonHub 提供后台联动和渠道相关工具。当前已支持基于以下项目的中转站账号：
...
- 🚀 **快速导出集成**  
  一键导出到 CherryStudio、CC Switch、CLIProxyAPI、Claude Code Router、Kilo Code，以及当前选择的自建托管站点（New API / DoneHub / Veloera / Octopus / AxonHub），减少复制粘贴与重复配置。

- 🔄 **自建站点后台联动**  
  面向自建 New API、DoneHub、Veloera、Octopus 与 AxonHub 的站点管理员，提供后台联动与渠道相关工具，减少频繁进后台操作。
