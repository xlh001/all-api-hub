...
export function FeedbackReportPreview({ content }: { content: string }) {
  ...
  return (
    <div
      ...
      ref={previewRef}
      className="text-secondary-foreground [&_a]:text-theme-600 dark:[&_a]:text-theme-400 [&_pre]:bg-surface-subtle dark:[&_pre]:bg-background [&_summary]:focus-visible:outline-ring [&_td]:border-border-subtle [&_th]:border-border-subtle [&_th]:text-muted-foreground max-w-none text-sm leading-6 break-words [&_a]:underline [&_details]:my-3 [&_h2]:mt-5 [&_h2]:mb-2 [&_h2]:text-sm [&_h2]:font-semibold [&_ol]:list-decimal [&_ol]:pl-5 [&_p]:my-2 [&_pre]:mt-3 [&_pre]:rounded-md [&_pre]:p-3 [&_pre]:text-xs [&_pre]:leading-5 [&_pre]:break-all [&_pre]:whitespace-pre-wrap [&_summary]:cursor-pointer [&_summary]:py-1 [&_summary]:text-sm [&_summary]:font-medium [&_summary]:focus-visible:outline-2 [&_table]:my-3 [&_table]:w-full [&_table]:table-fixed [&_table]:border-collapse [&_table]:text-xs [&_td]:border-b [&_td]:px-3 [&_td]:py-2 [&_td]:align-top [&_td]:break-words [&_th]:w-2/5 [&_th]:border-b [&_th]:py-2 [&_th]:text-left [&_th]:align-top [&_th]:font-medium [&_ul]:list-disc [&_ul]:pl-5 [&>:first-child]:mt-0"
    />
  )
}
